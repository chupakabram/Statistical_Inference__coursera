...
rownames(t1) <- c("Class of parameter", "Number of values"); 
t1
```
...
plot(tg, main="Fig.1. Scatterplot to illustrate data dependencies.",col="blue")
```
...
We suppose:      
 ...
 - there are `r length(tg)` rows in the data - the t-statistic should be used.     
...
The p-value is `r ttest$p.value` > $\alpha$ (0.05). It means we **can not reject** $H_0$ hypothesis, i.e we can't say with full confidence the parameter **supp** affects on **len**. 
...
boxplot(lnByDose05,lnByDose2, 
        main="Fig.3. Boxplot of 'len' distributions by 'dose'.", 
        names=c("Dose = 0.5", "Dose = 2.0"), col="lightgray")
...
The p-value is `r round(ttest$p.value)`, it less then $\alpha$ (0.05). We **have to reject** $H_0$ hypothesis, i.e  for **dose**=2.0 value of **len** is definitly high then for **dose**=0.5. 
